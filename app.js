...
function result()
{
	...
	for (var i=0; i<students.length; i++)
	{
		...
		{
			var avgmark=Math.round(Number(students[i].math)+Number(students[i].science)+Number(students[i].english));

			output.innerHTML='<h4>Maths:${students[i].math}</h4>
			                         <h4>Science:${students[i].science}</h4>
									 <h4>English:${students[i].english}</h4>
									 <h4>Average Marks:${avgmark}</h4>';
			break;
		}
		else
		{
